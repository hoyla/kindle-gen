...
class ArticleNITFSuite extends FunSuite {
  ...
  test("ArticleNITF apply") {
    val article = Article(
      ...
      issueDate = formatter.parseDate("20170724").toCapiDateTime,
      releaseDate = formatter.parseDate("20170724").toCapiDateTime,
      pubDate = formatter.parseDate("20170724").toCapiDateTime,
      byline = "my name",
