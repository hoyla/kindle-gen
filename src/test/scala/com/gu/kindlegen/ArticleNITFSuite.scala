...
class ArticleNITFSuite extends FunSuite {
  ...
  test("ArticleNITF apply") {
    val article = Article(
      ...
      issueDate = formatter.parseDateTime("20170724").toCapiDateTime,
      releaseDate = formatter.parseDateTime("20170724").toCapiDateTime,
      pubDate = formatter.parseDateTime("20170724").toCapiDateTime,
      byline = "my name",
